...
import { ArrowLeft, ChevronLeft, ChevronRight, History, LocateFixed, MapPin, Send, Trash2, X } from 'lucide-react'
            ...
            <div className="flex items-center justify-between text-slate-600">
              ...
              {currentClusterCapsules.length > 1 && (
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => {
                      const newIndex = activeClusterIndex > 0 ? activeClusterIndex - 1 : currentClusterCapsules.length - 1
                      setActiveClusterIndex(newIndex)
                      const target = currentClusterCapsules[newIndex]
                      if (target && mapInstanceRef.current) {
                        mapInstanceRef.current.setView([target.latitude, target.longitude], mapInstanceRef.current.getZoom(), { animate: true })
                      }
                    }}
                  ...
                  <button
                    onClick={() => {
                      const newIndex = activeClusterIndex < currentClusterCapsules.length - 1 ? activeClusterIndex + 1 : 0
                      setActiveClusterIndex(newIndex)
                      const target = currentClusterCapsules[newIndex]
                      if (target && mapInstanceRef.current) {
                        mapInstanceRef.current.setView([target.latitude, target.longitude], mapInstanceRef.current.getZoom(), { animate: true })
